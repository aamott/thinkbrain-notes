...
- resolution: Deferred — high cost, low reward. The two dialogs differ in focus management (DirtyCloseDialog saves/restores focus; DeleteConfirmDialog only focuses cancel on mount), backdrop behavior (none vs mousedown-close), and overlay styling (z-10/pt-[15vh]/bg-foreground/40 vs z-30/pt-[18vh]/bg-overlay). A shared ModalDialog primitive would need options for each, and the finding itself flags this as a future opportunity only when a third modal dialog is added. With only two consumers and substantial behavioral differences, extracting the shared component would add option-prop complexity without meaningful line savings.
