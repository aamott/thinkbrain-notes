...
Previously marked done in commit `a5f88cf`, then rolled back by `b2124ee "UI
Cleanup"` which deleted `apps/desktop/src/panels/` (BottomRegion, LeftPopout,
RightPopout) and the standalone `shell/{ActivityBar,TitleBar,StatusBar}.tsx`.
What remains is a single inlined `shell/DesktopShell.tsx` that composes the
shell without the panel-component separation this story requires. Reopened for
...
- `apps/desktop/src/shell/DesktopShell.tsx` (current inlined shell)
