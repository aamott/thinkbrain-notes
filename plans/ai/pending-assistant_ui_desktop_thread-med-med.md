...
Build a Tailwind-v4-themed assistant-ui chat thread for the right popout with
...
- [ ] A desktop `ThreadHistoryAdapter` persists `ThreadMessageLike` messages
      and metadata in OS app-data, keyed by thread ID, and round-trips format
      data without putting history in the vault.
