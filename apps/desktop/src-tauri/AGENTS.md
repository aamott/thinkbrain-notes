...
Tauri v2 Rust process managing native filesystem access, the workspace watcher, and Agent Client Protocol (ACP) host runtime.

## Module Map
- **`src/main.rs`**: Entry point for Tauri desktop executable.
- **`src/lib.rs`**: Rust library entry point and command handlers (FS, watcher, ACP host).
