...
/**
 * Placeholder chat model used until a real Agent Client Protocol runtime is wired in.
 *
 * The composer/thread UI is fully built out visually, but the runtime still throws
 * on any actual generation so we don't accidentally ship a half-wired agent.
 */
 ...
 *
 * This is a visual implementation: the thread + composer primitives come from
 * assistant-ui, but the runtime is still the placeholder {@link unconfiguredChatModel}.
 * The composer action bar (upload, settings popover, profile/LLM/harness selectors,
 * voice, send) and the status footer are presentational stubs.
 *
              ...
              {({ message }) => (
                ...
                >
                  <MessagePrimitive.Parts>
                    {() => <MessagePartPrimitive.Text className="block text-xs leading-relaxed" />}
                  </MessagePrimitive.Parts>
                </MessagePrimitive.Root>
              )}
            </ThreadPrimitive.Messages>
